...
function api_updateApplication(appId, data) {
  ...
  // 2. 権限チェック
  var ownerEmail = headerSheet.getRange(hRowIndex, HEADER_COL.APPLICANT_EMAIL).getValue();
  ...
  items.forEach(function (item) {
    ...
    // 画像データ(Base64)が新規添付されている場合の Upload処理はここでは割愛(FrontendでuploadReceiptForOcr済み前提)
    // ただし、もしFrontendで「再解析」などはせず画像だけ追加した場合、URLが無い可能性がある。
    // 今回の仕様では「OCR済み」としてreceiptUrlが入っている前提とする。
